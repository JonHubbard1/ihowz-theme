...
(function (blocks, element, blockEditor, components, i18n) {
    ...
    registerBlockType('ihowz/member-services-bento', {
        edit: function (props) {
            ...
            const blockProps = useBlockProps({
                className: 'ihowz-member-services-bento',
            ...
            // Check if card size supports images
            const supportsImage = (size) => size === 'medium' || size === 'large';
            ...
            return el(
                    ...
                    el(
                        ...
                        services.map((service, index) =>
                            el(
                                ...
                                el(SelectControl, {
                                    label: __('Card Size', 'ihowz-theme'),
                                    value: service.size || 'small',
                                    options: sizeOptions,
                                    onChange: (value) => updateService(index, 'size', value)
                                }),
                                el(SelectControl, {
                                ...
                                // Image upload for medium/large cards
                                supportsImage(service.size || 'small') && el(
                                    'div',
                                    { style: { marginTop: '15px', padding: '12px', background: '#f0f0f0', borderRadius: '4px' } },
                                    el('label', { style: { display: 'block', marginBottom: '8px', fontWeight: '600' } }, __('Card Image (Optional)', 'ihowz-theme')),
                                    el(
                                        MediaUploadCheck,
                                        null,
                                        el(MediaUpload, {
                                            ...
                                            render: ({ open }) => el(
                                                ...
                                                el(
                                                    ...
                                                    el(Button, {
                                                        ...
                                                        variant: 'secondary',
                                                        isSmall: true
                                                    }, service.imageUrl ? __('Replace Image', 'ihowz-theme') : __('Select Image', 'ihowz-theme')),
                                                    service.imageUrl && el(Button, {
                                                        onClick: () => {
                ...
                el(
                    ...
                    el(
                        'div',
                        { className: 'member-services-bento-container' },
                        // Header
                        el(
                            'div',
                            { className: 'member-services-bento-header' },
                            eyebrowText && el('span', { className: 'member-services-bento-eyebrow' }, eyebrowText),
                            heading && el('h2', { className: 'member-services-bento-heading' }, heading),
                            subheading && el('p', { className: 'member-services-bento-subheading' }, subheading)
                        ),
                        // Bento Grid
                        el(
                            'div',
                            { className: 'member-services-bento-cards' },
                            services.map((service, index) => {
                                const size = service.size || 'small';
                                const hasImage = supportsImage(size) && service.imageUrl;
                                const cardClass = 'member-services-bento-card member-services-bento-card--' + size + (hasImage ? ' has-image' : '');

                                return el(
                                    'div',
                                    { key: index, className: cardClass },
                                    // Content side
                                    el(
                                        'div',
                                        { className: 'member-services-bento-card-content' },
                                        el('div', { className: 'member-services-bento-card-icon' }, iconSvgs[service.icon] || iconSvgs.calendar),
                                        el('h3', { className: 'member-services-bento-card-title' }, service.title),
                                        el('p', { className: 'member-services-bento-card-description' }, service.description),
                                        service.buttonText && el(
                                            'a',
                                            { href: service.buttonUrl || '#', className: 'member-services-bento-card-btn' },
                                            service.buttonText
                                        )
                                    ),
                                    // Image side (only for medium/large with image)
                                    hasImage && el(
                                        'div',
                                        { className: 'member-services-bento-card-image' },
                                        el('img', { src: service.imageUrl, alt: service.title || '' })
                                    )
                                );
                            }),
